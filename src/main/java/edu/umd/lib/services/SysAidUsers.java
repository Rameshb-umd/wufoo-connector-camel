...
public class SysAidUsers {
   ...
   */
  private JSONArray LoadAllUsers() {

    try {

      SysAidConnector sysaid = new SysAidConnector("configuration.properties");
      HttpResponse response = sysaid.getRequest(sysaid.getSysaid_URL() + "users?view=mobile&offset=" + offsetValue);
      ...
      log.info("Length of Users" + list_response.length());
